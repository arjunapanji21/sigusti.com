...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Edit Materi Edukasi</h1>
            <p class="mt-2 text-sm text-gray-700">Perbarui informasi materi edukasi</p>
        </div>
    ...
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('materi.update', $materi) }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="judul" id="judul" value="{{ old('judul', $materi->judul) }}" required
                ...
                <div>
                    ...
                    <select name="kategori" id="kategori" required
                        ...
                        <option value="stimulasi" {{ old('kategori', $materi->kategori) == 'stimulasi' ? 'selected' : '' }}>Stimulasi</option>
                        <option value="nutrisi" {{ old('kategori', $materi->kategori) == 'nutrisi' ? 'selected' : '' }}>Nutrisi</option>
                        <option value="kesehatan" {{ old('kategori', $materi->kategori) == 'kesehatan' ? 'selected' : '' }}>Kesehatan</option>
                        <option value="tumbuh_kembang" {{ old('kategori', $materi->kategori) == 'tumbuh_kembang' ? 'selected' : '' }}>Tumbuh Kembang</option>
                    </select>
                    @error('kategori')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="usia_target" id="usia_target" value="{{ old('usia_target', $materi->usia_target) }}" placeholder="Contoh: 0-6 bulan, 6-12 bulan" required
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3" required
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm @error('deskripsi') border-red-300 @enderror">{{ old('deskripsi', $materi->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="konten" id="konten" rows="8" required
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm @error('konten') border-red-300 @enderror">{{ old('konten', $materi->konten) }}</textarea>
                    @error('konten')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($materi->gambar)
                        <div class="mt-1 mb-2">
                            <img src="{{ Storage::url($materi->gambar) }}" alt="Current image" class="h-20 w-20 object-cover rounded">
                ...
                <div>
                    ...
                    <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $materi->video_url) }}" placeholder="https://youtube.com/watch?v=..."
                ...
                <div class="md:col-span-2">
                    ...
                    @if($materi->file_attachment)
                        <div class="mt-1 mb-2">
                            <a href="{{ Storage::url($materi->file_attachment) }}" target="_blank" class="text-green-600 hover:text-green-900 text-sm">
